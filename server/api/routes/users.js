...
router.put("/:id", async (req, res, next) => {
  try {
    ...
    if(req.body.role){
      const {role} = req.body;
      if(role.roleId){
        await User_TeamRoles.create({userId: user.id, teamId: role.teamId, teamRoleId: role.roleId});
      } else {
        await User_LeagueRoles.create({userId: user.id, teamId: role.teamId, leagueRoleId: role.roleId})
      }
    } else {
